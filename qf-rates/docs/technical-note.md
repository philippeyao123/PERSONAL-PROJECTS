...
local optimum and G2++ parameters are not separately identifiable on sparse grids. The multi-start
API runs the default point plus seeded log-uniform points inside the bounds, retains every local
result and selects the lowest RMSE. Eight starts reduce the paper workload's RMSE from 0.00018688
to 0.00012551, demonstrating optimizer dependence without claiming identification. Regularization
and denser grids remain necessary before interpreting parameters economically. The selected observed
solution reaches the 120-iteration cap, places correlation at -0.938125 near the lower bound, and
has a maximum absolute quote residual of 2.176 normal-volatility basis points. These diagnostics
are retained because the smallest scalar RMSE is not a convergence or uniqueness certificate.
...
release uses a disclosed European floor to protect the essential lower bound. The separate
out-of-sample engine fits coefficients on training paths, freezes the exercise policy, and values it
forward on paths from a distinct seed. It also records paths that never exercise. Across the paper
grid, independent prices span 0.01376436--0.01389826 and training-minus-valuation gaps change sign.
This removes direct path reuse but not regression approximation error. Production work should use
QR or SVD, richer normalized states, cross-fitting and a dual upper bound that brackets the true
Bermudan value.

### 21.1 Coupled Monte-Carlo time discretization

An ordinary comparison between different time-step counts mixes sampling noise with integration
bias. The coupled engine instead simulates factor states once on a 384-step grid and evaluates
trapezoidal discount integrals on nested 12, 24, 48, 96, 192 and 384-step grids. Terminal states and
payoffs are identical at every level. Online statistics of pathwise price differences provide a
paired bias and paired standard error. On seeds 7 and 42 with 20,000 physical paths, the largest
absolute bias versus the finest grid is \(6.79\times10^{-8}\) price units.

### 21.2 G2++ stress grid

The research benchmark evaluates five parameter regimes (low volatility, base, high volatility,
fast mean reversion and weak correlation), expiries 1Y/2Y/5Y, tenors 2Y/5Y/10Y and payer strikes
at -100/0/+100 basis points from par. The resulting 135 deterministic cells are committed as CSV.
Tests require finite prices, strike monotonicity and non-decreasing value when both factor
volatilities rise with other parameters fixed. This broadens internal coverage but does not replace
an equally broad external-library comparison.
...
The publication benchmark retains the full grid beta \(=0,5,\ldots,30\), together with EPE at
0.5Y, 1Y, 2Y, 3Y, 4Y and 5Y. CVA rises monotonically to 1857.27799608 at beta 30, a 41.5% increase
over the baseline. The grid is a sensitivity diagnostic, not a beta calibration.
