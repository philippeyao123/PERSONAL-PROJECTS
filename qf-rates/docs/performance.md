...
|---|---|---|---:|
| Release (`-O2`/CMake Release) | Apple arm64, macOS | AppleClang 21.0.0 | 571 prices/s |

Observed wall time was 1.753 seconds for 1,000 prices. This is a development-machine baseline, not a
cross-platform performance guarantee. Reproduce it with:

```bash
cmake -S . -B build -DCMAKE_BUILD_TYPE=Release -DQF_BUILD_BENCHMARKS=ON
cmake --build build --target qf_rates_bench qf_rates_validation --parallel
./build/qf_rates_bench
./build/qf_rates_validation
```
...
`qf_rates_validation` is deliberately separate from the throughput micro-benchmark. It records
variance-reduction ratios, the LSM path/seed/basis grid and the WWR comparison used in the technical
note. This separation prevents a speed result from being mistaken for evidence of numerical
quality.
