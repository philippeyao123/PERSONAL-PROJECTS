...
| MC time grid | coupled 12--384-step discount integrals | paired bias and standard error |
| G2++ stress grid | 135 finite cells; strike/volatility monotonicity | strict property |
| G2++ external grid | 135 QuantLib cells across regimes, expiries, tenors and strikes | distribution reported |
| Cross-engine risk | fixed-strike curve DV01 and joint model-volatility bump | distribution reported |
| Calibration | eight starts; finite RMSE and constrained parameters | best RMSE below 0.001 |
| LSM | one date equals European; independent fixed-policy valuation | \(10^{-14}\), statistical |
...
With the optional bindings and QuantLib-Python it compares a vanilla swap, a
135-cell G2++ European swaption surface, four finite-difference risk cases, and
a six-quote normal-volatility calibration. CI requires these optional
dependencies and fails when a required check is unavailable.
...
On 2026-07-29 the script was run against QuantLib-Python 1.43 and pybind11 3.0.4. Analytic options
matched to all 12 printed decimal places. Swap par rate and NPV matched at machine precision. The
anchor G2++ European price differed by 0.720343%. Across 123 material cells in
the systematic grid, the median and 95th-percentile absolute relative
differences were 0.184% and 2.486%. The largest discrepancies in the four-case
risk study were 4.69% for fixed-strike curve DV01 and 1.38% for a joint
model-volatility bump.
...
The time-discretization experiment simulates each path once on 384 steps and evaluates trapezoidal
discount integrals on nested 12, 24, 48, 96, 192 and 384-step grids. Because terminal factors and
payoffs are shared pathwise, the reported paired bias isolates integration error from ordinary
payoff sampling noise. Across seeds 7 and 42, the maximum absolute paired bias relative to the
384-step grid is \(6.79\times10^{-8}\) price units.

The complete deterministic G2++ grid contains 135 cells: five parameter regimes, three expiries,
three tenors and three moneyness levels. Automated tests require finite values, decreasing payer
prices with strike, and non-decreasing values when both factor volatilities increase.

Eight seeded calibration starts reduce the best six-quote normal-volatility RMSE from 0.00018688
at the default start to 0.00012551. Every initial point and local result is retained; the
improvement is evidence of optimizer dependence, not parameter identification.

Out-of-sample LSM fits a policy on 2,000, 5,000 or 10,000 training paths and evaluates frozen
coefficients on 10,000 paths from a distinct seed. Independent values span
0.01376436--0.01389826. The training-minus-valuation difference changes sign across seeds, so the
result is reported as policy uncertainty rather than assumed positive look-ahead bias.
