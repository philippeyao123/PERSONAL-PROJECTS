...
\hypersetup{
  ...
  pdftitle={European Power Fair Value: Reproducible Day-Ahead Forecasting for DE-LU},
...
\title{\texttt{European Power Fair Value}:\\
Reproducible Day-Ahead Forecasting and Prompt-Proxy Diagnostics\\
for the Germany--Luxembourg Market}
...
Three point forecasts are evaluated on a strict expanding-window experiment
...
MAE of \NaiveMAE{} EUR/MWh, Ridge obtains \RidgeMAE{}, and deterministic
LightGBM obtains \PrimaryMAE{} with a delivery-day block-bootstrap 95\%
interval of [\PrimaryMAELow{}, \PrimaryMAEHigh{}].  The corresponding
improvements are \SkillNaive{} relative to the naive model and \SkillRidge{}
relative to Ridge.  Daily absolute-loss differentials remain significant
under a seven-lag heteroskedasticity- and autocorrelation-consistent variance
estimate.  Feature-family ablations show that removing day-ahead renewables
...
mean width \ConformalWidth{} EUR/MWh.  Coverage nevertheless falls below the
target during several daylight and evening hours, and tail diagnostics show
that MAE reaches \NegativeMAE{} on negative-price hours and \HighMAE{} above
200 EUR/MWh.  A separate prompt-proxy experiment maps next-day baseload fair
value into directional front-week views.  At the declared threshold it
contains \SignalActive{} evaluable active days, a \SignalHit{} hit rate, and a
mean signed spread of \SignalCaptured{} EUR/MWh; moving-block intervals are
reported.  This last experiment is descriptive and is not presented as
tradable P\&L because the prompt input is a trailing day-ahead proxy rather
than a licensed forward settlement.
...
\begin{enumerate}[leftmargin=*,itemsep=2pt]
  ...
  forecast and a regularized linear baseline over a complete out-of-sample
  year?
...
\begin{enumerate}[leftmargin=*,itemsep=2pt]
  ...
  with three baselines under a shared information set.
  ...
  errors, extreme-price detection rates, and prequential interval coverage.
...
outperforms its two declared baselines on this declared sample, and that the
...
baseline is particularly informative in negative-price hours, where the
nonlinear model is not uniformly superior.
...
intervals for MAE resample entire daily summaries rather than individual
hours.
...
guarantee under arbitrary temporal dependence.
...
public-power-forecast endpoint.  Fifteen-minute prices, when present, are
...
weather observations are initially missing, but they occur before the maximum
price lag has formed and do not enter the modeled sample.
...
local delivery day and resamples the 365 daily values with replacement 4,000
times.  The 2.5 and 97.5 percentiles form the reported interval.  This
preserves within-day dependence but treats days as exchangeable for the
bootstrap; the resulting interval should not be read as robust to arbitrary
long regime shifts.
...
\]
This is a Diebold--Mariano-style comparison on daily $L_1$ loss.  The
$t$ reference distribution and fixed lag are approximations, so exact
$p$-values should not be interpreted more finely than their overwhelming
order of magnitude.
...
moving-block bootstrap is therefore used for the default $(w,\tau)=(7,0.75)$
...
MAE of \PrimaryMAE{} EUR/MWh, against \RidgeMAE{} for Ridge and \NaiveMAE{}
for the weekly naive forecast.  The primary model's RMSE is
...
\begin{table}[t]
\centering
\caption{One-year expanding-window point-forecast accuracy.  MAE intervals
resample complete delivery-day summaries 4,000 times.}
\label{tab:models}
\small
\resizebox{\textwidth}{!}{\input{generated/model_comparison_table.tex}}
\end{table}

\begin{figure}[t]
\centering
\includegraphics[width=0.92\textwidth]{07_model_uncertainty.pdf}
\caption{MAE and delivery-day block-bootstrap 95\% intervals.  Intervals
...
The daily MAE gain relative to the naive model is 19.29 EUR/MWh; relative to
Ridge it is 3.52 EUR/MWh.  Both autocorrelation-robust tests reject a zero mean
...
Positive gains favor LightGBM.}
\label{tab:dm}
\small
\input{generated/dm_table.tex}
\end{table}
...
are harder.  The nonlinear model's MAE is 9.20 EUR/MWh during hours 00--06,
16.28 during hours 07--15, and 15.30 during hours 16--21.  Spring has the
largest seasonal MAE at 15.82, followed by autumn at 14.27, summer at 11.94,
and winter at 11.39.

\begin{figure}[t]
\centering
\includegraphics[width=0.84\textwidth]{04_mae_by_hour.pdf}
\caption{MAE by local delivery hour for all three models.}
\label{fig:hour}
\end{figure}
...
inside the broad day-bootstrap intervals and does not justify relaxing the
...
renewables and calendar variables but no lagged prices obtains MAE 17.80,
which remains substantially better than price-history plus calendar at 25.14.
...
\label{tab:ablation}
\small
\resizebox{\textwidth}{!}{\input{generated/ablation_table.tex}}
\end{table}
...
above-200 EUR/MWh hours.  On regular hours, LightGBM obtains MAE 12.14;
...
fuel-stack variables, or train an explicit tail objective.
...
\label{tab:tails}
\small
\input{generated/tail_table.tex}
\end{table}
...
Across 8,040 evaluable hours after the minimum calibration period, the nominal
90\% interval covers \ConformalCoverage{} of outcomes with mean width
\ConformalWidth{} EUR/MWh.  Aggregate calibration is close to the target.
...
The aggregate number conceals intraday miscalibration.
Figure~\ref{fig:coverage-hour} shows coverage near 95--97\% overnight and late
in the evening, but 84--89\% for many daylight hours.  A common radius is too
wide in easy hours and too narrow in difficult hours.  Hour-conditional
residual pools, quantile regression, or a dynamic conformal method are
natural extensions.  Because the time series is nonexchangeable, the reported
empirical rate remains a descriptive validation result rather than a
finite-sample coverage theorem.

\begin{figure}[t]
\centering
\includegraphics[width=0.84\textwidth]{11_conformal_coverage.pdf}
\caption{Empirical interval coverage by local delivery hour.}
...
hit rate and magnitude while sharply reducing the sample.  For the seven-day
proxy, the active count falls from 200 near threshold 0.5 to about
twenty near threshold 2.0.  The best-looking ex-post cell is therefore not
evidence for an optimized live threshold.  The value 0.75 is retained because
it was declared in the prototype before this expanded analysis.
...
and Ridge.  The improvement appears through the year, survives daily
dependence adjustments, and is not explained by the ambiguous-vintage weather
series.  Renewable forecasts are a central source of skill.
...
the operational boundary of the model.
...
Prompt-proxy sensitivity & \texttt{trading.py}, \texttt{research.py} &
...
The test suite checks price-lag direction, QA invariants, the sign convention
of the HAC comparison, strictly prior conformal calibration, the exact
365-day OOS shape, prompt right-censoring, equality between primary ablation
and frozen model predictions, coverage reconciliation, dataset hashing, and
the presence of all fourteen vector figures.
...
\begin{enumerate}[leftmargin=*,itemsep=5pt]
  ...
  crises, policy regimes, or fuel cycles.
  ...
  90\% coexists with conditional undercoverage, and temporal dependence
  prevents a naive exchangeability guarantee.

  \item \textbf{Significance.}  The HAC lag, day bootstrap, and reference
  distribution are modeling choices.  Extremely small $p$-values indicate
  strong sample evidence, not an immutable population ranking.
  ...
  outcomes, missing costs, and threshold exploration preclude a trading
  performance claim.
...
\PrimaryMAE{} EUR/MWh MAE and improves on both a weekly naive model and Ridge.
The gain is stable across the sample and significant under a daily
autocorrelation-robust comparison.  Renewable forecasts account for a large
share of predictive value.  The more permissive weather variant changes MAE
only marginally and is excluded from the primary claim because its archive
does not preserve a fixed D-1 vintage.

The same evidence also limits the conclusion.  Error remains large in
negative and above-200 price regimes, daytime intervals undercover, and the
sample covers one recent year rather than multiple structural regimes.  The
prompt-proxy experiment is statistically transparent but not a forward-market
backtest.
...
probabilistic forecasts would turn that foundation into a broader empirical
study.
...
\begin{enumerate}[leftmargin=*]
  ...
  and deterministic LightGBM on the training mask.
  \item Predict every 23/24/25 target hour of $D_i$ with the same fitted
  models; copy the 168-hour lag as the naive prediction.
  \item Append timestamp, target, and all three predictions without shuffling.
...
\begin{enumerate}[leftmargin=*]
  ...
  main-model column.
  \item Compute metrics and delivery-day bootstrap intervals for every set.
\end{enumerate}
...
quartiles and all three models.  For the primary model, MAE is 13.82, 9.98,
12.48, and 18.09 EUR/MWh across increasing renewable quartiles.  The
